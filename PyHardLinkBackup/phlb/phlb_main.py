...
from PyHardLinkBackup.phlb.human import (
    human_time,
    human_filesize,
    to_percent,
    ns2naturaltimesince,
...
def backup(path, name):
    ...
    with path_helper.summary_filepath.open("w") as f:
        ...
        try:
            ...
            summary("Abort backup, because user hits the interrupt key during execution!", flush=True)
            # Calculate the correct omitted files count for print_summary()
            phlb.total_errored_items = phlb.file_count - (phlb.total_file_link_count + phlb.total_new_file_count)
        except Exception:
...
def scan_dir_tree(path, extra_skip_patterns=None):
    ...
    print("\nscan/filter source directory in %s" % (human_time(default_timer() - start_time)))
    return filtered_dir_entries
