...
class TestTwoBackups(BaseCreatedOneBackupsTestCase):
    ...
    def test_hash_file_not_found(self):
        file_path = Path2(self.first_run_path, "sub dir A", "dir_A_file_B.txt.%s" % phlb_config.hash_name)
        file_path.unlink()
        ...
        self.assertIn("ERROR", result.output)
        self.assertIn("Verify done.", result.output)
        self.assertIn("Hash file not found: %s" % file_path.path, result.output)
