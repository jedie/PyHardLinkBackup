...
def iter_scandir_files(path: Path, follow_symlinks=True) -> Iterable[os.DirEntry]:
    for entry in os.scandir(path):
        if entry.is_file(follow_symlinks=follow_symlinks):
            yield entry
        elif entry.is_dir(follow_symlinks=follow_symlinks):
            yield from iter_scandir_files(Path(entry.path), follow_symlinks=follow_symlinks)
